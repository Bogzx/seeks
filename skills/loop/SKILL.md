...
3. **Backlog EMPTY** → decide by `min_dry_sweeps` (in status.json):
   ...
   - **`min_dry_sweeps` > 0 (an until-dry loop, e.g. "fix all bugs")** → first run **one discovery sweep**: re-run the analyzers / re-trace the goal's blast radius for *new* issues, then `… sweep-tick <name> <count-found>`, then **end the pass** unless you're ready to verify:
