...
3. **Backlog EMPTY** → decide by `min_dry_sweeps` (in status.json):
   ...
   - **`min_dry_sweeps` > 0 (an until-dry loop, e.g. "fix all bugs")** → first run **one creative discovery sweep**: actually READ the source across the goal's blast radius and reason about correctness like a skeptical reviewer — hunt for *real* defects the tests don't catch (logic / off-by-one, wrong operator or boundary, unhandled `None`/empty/error paths, bad parsing, timezone/encoding/locale bugs, resource leaks, races, auth/injection gaps). **Re-running tests/linters is only a regression floor, NOT the bug finder.** For fresh eyes, dispatch a bug-hunter subagent so it isn't anchored on your own edits. Then `… sweep-tick <name> <count-of-NEW-real-bugs>`, then **end the pass** unless ready to verify:
   ...
   **Verifier subagent** (separate context, maker ≠ checker): hand it the absolute worktree path; it must `cd` there and RUN each executable done-condition from spec.md itself (subagents do NOT inherit cwd), write `verify/<n>.md`, and report per condition.
   - **All non-human conditions pass →** `… status-set <name> '{"verifier_certified":true,"done":true,"last_verdict":"pass"}'`. (The gate still won't release until `dry_sweeps ≥ min_dry_sweeps`.)
