...
3. **Backlog EMPTY** → decide by `min_dry_sweeps` (in status.json):
   ...
   - **`min_dry_sweeps` > 0 (an until-dry loop, e.g. "fix all bugs")** → first run **one creative discovery sweep, through a fresh lens each pass**. Get the angle: `LENS=$(… sweep-next-lens <name>)` (rotates: concurrency, error-handling, boundary, resource-lifecycle, serialization, input-trust). Then actually READ the source across the goal's blast radius and reason about correctness **specifically through `$LENS`** — hunt for *real* defects the tests don't catch. **Re-running tests/linters is only a regression floor, NOT the bug finder.** For fresh eyes, dispatch a bug-hunter subagent so it isn't anchored on your own edits. Then `… sweep-tick <name> <count-of-NEW-real-bugs> "$LENS"`, then **end the pass** unless ready to verify. **Note:** a dry sweep only advances `dry_sweeps` if its lens is *new* to the current dry streak — repeating a lens won't count, so always take the lens `sweep-next-lens` gives you.
